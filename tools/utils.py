...
def read_annotations(data_path):
    ...
    for line in lines:
        ...
        landmarks = [float(each) for each in line.split()[1:]]

        data.append((sample_path, landmarks))
    return data
...
def get_train_paths(args):
    ...
    model_dir = os.path.join(args.data_path, args.train_collection, "models", args.val_collection, args.prefix,
                             args.config_name, "run_%d" % args.run_id)
